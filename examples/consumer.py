...
def create_app():
        ...
        def decorated_function(*args, **kwargs):
            if get_oauth_token() is None:
                ...
                return (
                    ...
                    '<a href="{}"> Get one from here</a>'.format(url_for("index"))
                )
    ...
    def login():
        ...
        """
        return remote.authorize(callback_uri=url_for("authorized", _external=True))
    ...
    def authorized():
        ...
        if response is None:
            return (
                ...
                "<br>"
                "<pre>error={error}</pre>"
                '<br><a href="{link}"> Try again</a>'.format(
                    link=url_for("index"), error=request.args["error"]
                )
            )
